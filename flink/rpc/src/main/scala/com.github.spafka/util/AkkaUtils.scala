...
object AkkaUtils {
  ...
  @throws[Exception] def startActorSystem(confPath: String, logger: Logger): ActorSystem = {
    try {
      ...
      val actorSystem = ActorSystem.create("master", config)
  ...
  @throws[Exception] def startMasterActorSystem(confPath: String = "master.conf", logger: Logger): ActorSystem = {
    startActorSystem(confPath, logger)
  }

  @throws[Exception] def startSlaveActorSystem(confPath: String = "slave.conf", logger: Logger): ActorSystem = {
    startActorSystem(confPath, logger)
  }
  ...
  @VisibleForTesting def startDebugActorSystem(): ActorSystem = {
    try {
      import akka.actor.ActorSystem
      val actorSystem = ActorSystem.create("flink")
      actorSystem
    } catch {
