...
public abstract class AbstractStreamOperator<OUT>
    ...
    public final OperatorSnapshotResult snapshotState(long checkpointId, long timestamp, CheckpointOptions checkpointOptions) throws Exception {
        ...
        try (StateSnapshotContextSynchronousImpl snapshotContext = new StateSnapshotContextSynchronousImpl(
            ...
            LOG.warn("[CHECKPOINT] step 1，把数据写入statebacked");
            ...
            if (null != operatorStateBackend) {
                LOG.warn("[operatorStateBackend CHECKPOINT] step 2，把数据写入statebacked copy to remote");
                snapshotInProgress.setOperatorStateManagedFuture(
                        operatorStateBackend.snapshot(checkpointId, timestamp, factory, checkpointOptions));
            }
            if (null != keyedStateBackend) {

                LOG.warn("[keyedStateBackend CHECKPOINT] step 2，把数据写入statebacked copy to remote");
