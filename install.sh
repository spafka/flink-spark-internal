...
HadoopLink='http://mirrors.hust.edu.cn/apache/hadoop/common/hadoop-2.7.7/hadoop-2.7.7.tar.gz'
function installJDK        #安装JDK
{
...
function InstallHadoop        #安装hadoop
{
        cd ~
        ls -l | grep hadoop-2.7.7.tar.gz
        if [ $? -ne 0 ] ; then
...
sed -ie 's/export JAVA_HOME=${JAVA_HOME}/export JAVA_HOME=\/usr\/local\/java/g' /usr/local/hadoop/etc/hadoop/hadoop-env.sh
        source /etc/profile
        cd /usr/local/hadoop
        ./bin/hadoop version
...
function setSSHWithoutCode        #设置ssh免密登陆
{
