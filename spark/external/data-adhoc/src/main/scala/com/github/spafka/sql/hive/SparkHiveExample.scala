 ...
 */
package com.github.spafka.sql.hive
...
object SparkHiveExample {
  ...
  def main(args: Array[String]) {
    ...
    spark.sqlContext.setConf("hive.exec.dynamic.partition.mode", "nonstrict") // Create a Hive partitioned table using DataFrame API
    df.write.partitionBy("key").format("hive").saveAsTable("hive_part_tbl") // Partitioned column `key` will be moved to the end of the schema.
    ...
    // |val_311|311|
    // ...
    spark.stop() // $example off:spark_hive$
  }
}
