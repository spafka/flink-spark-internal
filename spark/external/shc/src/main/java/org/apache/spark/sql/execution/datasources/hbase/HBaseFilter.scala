...
case class HRF[T](ranges: Array[ScanRange[T]],
                  tf: TypedFilter,
...
object HBaseFilter extends Logging {
  ...
  def buildFilters(filters: Array[Filter],
                   relation: HBaseRelation): HRF[Array[Byte]] = {
    ...
    val ret =
      filter.map(buildFilter(_, relation)).getOrElse(HRF.empty[Array[Byte]])
    if (log.isDebugEnabled) {
      logDebug("ret:")
  ...
  def buildFilter(filter: Filter, relation: HBaseRelation): HRF[Array[Byte]] = {
    ...
    def Greater(attribute: String, value: Any): HRF[Array[Byte]] = {
      process(
          ...
          if (relation.singleKey) {
            HRF(
              bound.greater.map(
                x =>
                  ScanRange(
                    Some(Bound(x.low, true)),
                    Some(Bound(x.upper, true))
                )
              ),
              TypedFilter.empty
            )
        ...
        bound => {
          ...
          val of = filter.reduce[TypedFilter] {
            case (x, y) => TypedFilter.or(x, y)
          }
    ...
    def Less(attribute: String, value: Any): HRF[Array[Byte]] = {
      process(
        ...
        bound => {
          ...
          val ob = filter.reduce[TypedFilter] {
            case (x, y) => TypedFilter.or(x, y)
          }
    ...
    def setDiff(inValues: Array[Any],
                notInValues: Array[Any],
                attrib: String): HRF[Array[Byte]] = {
    ...
    val f = filter match {
      case And(
          ...
          if inAttrib == notInAttrib => //this is set difference being performed
      ...
      case And(left, right) =>
        and[Array[Byte]](
          buildFilter(left, relation),
          buildFilter(right, relation)
        )
      case Not(And(left, right)) =>
        or[Array[Byte]](
          buildFilter(Not(left), relation),
          buildFilter(Not(right), relation)
        )
      case Or(left, right) =>
        or[Array[Byte]](
          buildFilter(left, relation),
        ...
        process(
          ...
          bound => {
            ...
            HRF(
              ...
              TypedFilter(Some(filter), FilterType.Atomic),
              true
            )
          },
          bound => {
      ...
      case StringStartsWith(attribute, value) =>
        val b = SHCDataTypeFactory
          .create(relation.getField(attribute).fCoder)
          .toBytes(value)
        if (relation.isPrimaryKey(attribute)) {
      ...
      case Not(StringContains(attribute: String, value: String))
          if relation.isColumn(attribute) =>
        val f = relation.getField(attribute)
        val filter = new SingleColumnValueFilter(
          tCoder.toBytes(f.cf),
      ...
      case Not(In(attribute: String, values: Array[Any])) => //converting a "not(key in (x1, x2, x3..)) filter to (key != x1) and (key != x2) and ..
        val hrf = values
          .map { v => buildFilter(Not(EqualTo(attribute, v)), relation)
          }
          .reduceOption[HRF[Array[Byte]]] { case (lhs, rhs) => and(lhs, rhs) }
          .getOrElse(HRF.empty[Array[Byte]])
        HRF(hrf.ranges, hrf.tf, handled = false)
      case _ => HRF.empty[Array[Byte]]
    }
    logDebug(
      s"""start filter $filter:  ${f.ranges.map(_.toString).mkString(" ")}"""
    )
    f
  }

  def and[T](left: HRF[T],
