...
object SchemaConverters {
  ...
  def toSqlType(avroSchema: Schema): SchemaType = {
    avroSchema.getType match {
      ...
      case other =>
        throw new SchemaConversionException(s"Unsupported type $other")
    }
  }
  ...
  private def convertSparkSQLTypeToAvro[T](dataType: DataType,
    ...
    dataType match {
      ...
      case ArrayType(elementType, _) =>
        val builder = getSchemaBuilder(
          dataType.asInstanceOf[ArrayType].containsNull
        )
        val elementSchema = convertSparkSQLTypeToAvro(
          elementType,
          builder,
          structName,
          recordNamespace
      ...
      case MapType(StringType, valueType, _) =>
        val builder = getSchemaBuilder(
          dataType.asInstanceOf[MapType].valueContainsNull
        )
        val valueSchema = convertSparkSQLTypeToAvro(
          valueType,
          builder,
          structName,
          recordNamespace
      ...
      case other =>
        throw new IllegalArgumentException(s"Unexpected type $dataType.")
    }
  }
  ...
  private def convertSparkStructTypeToAvro[T](structType: StructType,
    ...
    dataType match {
      ...
      case ArrayType(elementType, _) =>
        val elementConverter =
          createConverterToAvro(elementType, structName, recordNamespace)
        (item: Any) => {
          if (item == null) {
            null
          } else {
      ...
      case MapType(StringType, valueType, _) =>
        val valueConverter =
          createConverterToAvro(valueType, structName, recordNamespace)
        (item: Any) => {
          if (item == null) {
            null
          } else {
            ...
            item.asInstanceOf[Map[String, Any]].foreach {
              case (key, value) => javaMap.put(key, valueConverter(value))
            }
            javaMap
          }
        }
  ...
  def createConverterToSQL(schema: Schema): Any => Any = {
    schema.getType match {
      ...
      case INT | BOOLEAN | DOUBLE | FLOAT | LONG =>
        identity // Byte arrays are reused by avro, so we have to make a copy of them.
      case FIXED =>
        (item: Any) =>
          if (item == null) {
            null
      ...
      case other =>
        throw new SchemaConversionException(s"invalid avro type: $other")
    }
  }
}
...
object AvroSerde {
  ...
  def serialize(input: Any, schema: Schema): Array[Byte] = {
    ...
    try {
      ...
      case e: IOException =>
        throw new Exception(
          s"Exception while creating byte array for Avro schema ${schema}"
        )
    } finally {
      if (bao != null) {
        bao.close()
