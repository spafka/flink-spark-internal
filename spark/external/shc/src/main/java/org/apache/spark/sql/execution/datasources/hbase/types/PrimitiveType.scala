...
class PrimitiveType(f: Option[Field] = None) extends SHCDataType {
      ...
      f.get.dt match {
        ...
        case _ =>
          throw new UnsupportedOperationException(
            s"unsupported data type ${f.get.dt}"
          )
      }
    } else {
      throw new UnsupportedOperationException(
        "PrimitiveType coder: without field metadata, " + "'fromBytes' conversion can not be supported"
      )
    }
  }
  ...
  def toBytes(input: Any): Array[Byte] = {
    input match {
      ...
      case _ =>
        throw new UnsupportedOperationException(
          s"PrimitiveType coder: unsupported data type $input"
        )
    }
  }
  ...
  private def fromBytes(field: Field,
                        src: HBaseType,
                        offset: Int,
                        length: Int): Any = {
    field.dt match {
      ...
      case _ =>
        throw new UnsupportedOperationException(
          s"PrimitiveType coder: unsupported data type ${field.dt}"
        )
    }
  }
  ...
  private def toUTF8String(input: HBaseType,
                           length: Int,
                           offset: Int = 0): UTF8String = {
