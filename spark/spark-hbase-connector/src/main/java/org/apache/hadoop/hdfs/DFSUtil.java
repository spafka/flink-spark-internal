...
public class DFSUtil {
    ...
    public static Collection<URI> getNameServiceUris(Configuration conf,
        ...
        for (String key : keys) {
            ...
            if (addr != null) {
                URI uri = createUri("org/apache/hadoop/hdfs", NetUtils.createSocketAddr(addr));
                if (!uriFound) {
                    uriFound = true;
                    ret.add(uri);
                } else {
    ...
    public static KeyProvider createKeyProvider(Configuration conf) throws IOException {
        String providerUriStr = conf.get("dfs.encryption.key.provider.uri", (String)null);
        if (providerUriStr == null) {
            ...
            } else if (keyProvider.isTransient()) {
